...
export type CartEvent = 
  | { type: 'ADD_ITEM'; payload: { item: MenuItem; restaurant: Restaurant; quantity?: number; note?: string } }
...
interface CartContextType extends CartState {
  ...
  showCart: () => void;
  hideCart: () => void;
  addToCart: (item: MenuItem, restaurant: Restaurant, quantity?: number, note?: string) => void;
...
function cartReducer(state: CartState, event: CartEvent): CartState {
  switch (event.type) {
    ...
    case 'REMOVE_ITEM': {
      ...
      const newLines = state.cart.lines.filter(line => line.id !== event.payload.lineId);
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  const addToCart = (item: MenuItem, restaurant: Restaurant, quantity = 1, note?: string) => {
    dispatch({ 
      type: 'ADD_ITEM', 
