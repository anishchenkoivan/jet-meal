...
SAMPLE_CREATE_BODY='{"businessName":"Test Cafe","ownerUserId":"user-1","description":"A test cafe","businessLogoUrl":"http://example.com/logo.png"}'
...
@test "update existing business returns 204" {
  id=$(post_v1_business "$SAMPLE_CREATE_BODY" | grep -o '"business_id":"[0-9]*"' | grep -o '[0-9]*')
  status=$(put_v1_business_status "{\"businessId\":\"$id\",\"businessName\":\"Updated Cafe\",\"ownerUserId\":\"user-1\",\"description\":\"Updated\",\"businessLogoUrl\":\"http://example.com/new.png\"}")
  [ "$status" = "204" ]
}

@test "update nonexistent business returns 404" {
  status=$(put_v1_business_status '{"businessId":"999999999","businessName":"X","ownerUserId":"u","description":"d","businessLogoUrl":"http://x.com/l.png"}')
  [ "$status" = "404" ]
}
...
@test "get after update reflects new values" {
  id=$(post_v1_business "$SAMPLE_CREATE_BODY" | grep -o '"business_id":"[0-9]*"' | grep -o '[0-9]*')
  put_v1_business_status "{\"businessId\":\"$id\",\"businessName\":\"New Name\",\"ownerUserId\":\"user-2\",\"description\":\"New desc\",\"businessLogoUrl\":\"http://example.com/new.png\"}" > /dev/null
  response=$(get_v1_business "{\"businessId\":\"$id\"}")
